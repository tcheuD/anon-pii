...
impl HeuristicNerDetector {
    ...
    fn is_name_word(word: &str) -> bool {
        ...
        if word.chars().all(|c| c.is_uppercase() || c == '-' || c == '\'')
    ...
    fn detect_dictionary_patterns(&self, text: &str) -> Vec<NerSpan> {
        ...
        for i in 0..words.len() {
            ...
            if self.first_names.contains(clean) {
                ...
                let leading_stripped = word.len() - word.trim_start_matches(|c: char| c.is_ascii_punctuation()).len();
                ...
                } else if name_parts.len() == 1 && self.has_context(text, clean_start, clean_start + clean.len()) {
...
mod tests {
    ...
    fn test_allcaps_lastname_with_title() {
        ...
        assert_eq!(spans.len(), 1, "Should detect MARTIN after title: {:?}", spans);
        assert_eq!(spans[0].text, "MARTIN");
    }
    ...
    fn test_insee_names_loaded() {
        ...
        assert!(INSEE_FIRST_NAMES.len() >= 2000, "Expected 2000+ INSEE names, got {}", INSEE_FIRST_NAMES.len());
        ...
        assert!(names.contains("Rachida"), "Rachida should be in INSEE names");
        assert!(names.contains("Jean-Pierre"), "Jean-Pierre should be in INSEE names");
        assert!(names.contains("Abdoulaye"), "Abdoulaye should be in INSEE names");
    }
    ...
    fn test_insee_name_detection_previously_missed() {
        ...
        assert_eq!(spans.len(), 1, "Rachida BENALI should be detected: {:?}", spans);
        ...
        assert_eq!(spans.len(), 1, "Abdoulaye DIALLO should be detected: {:?}", spans);
        ...
        assert_eq!(spans.len(), 1, "Nadia FOURNIER should be detected: {:?}", spans);
