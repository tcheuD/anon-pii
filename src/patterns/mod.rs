...
pub const PATTERNS: &[PiiPattern] = &{
    ...
    const fn build_patterns() -> [PiiPattern; TOTAL_LEN] {
        ...
        while j < IT_PATTERNS.len() {
            result[i] = PiiPattern {
                name: IT_PATTERNS[j].name,
                entity_type: IT_PATTERNS[j].entity_type,
                pattern: IT_PATTERNS[j].pattern,
                score: IT_PATTERNS[j].score,
                context_keywords: IT_PATTERNS[j].context_keywords,
                context_required: IT_PATTERNS[j].context_required,
            };
            i += 1;
            j += 1;
        }
        ...
        // KR patterns
        j = 0;
        while j < KR_PATTERNS.len() {
            result[i] = PiiPattern {
                name: KR_PATTERNS[j].name,
                entity_type: KR_PATTERNS[j].entity_type,
                pattern: KR_PATTERNS[j].pattern,
                score: KR_PATTERNS[j].score,
                context_keywords: KR_PATTERNS[j].context_keywords,
                context_required: KR_PATTERNS[j].context_required,
            };
            i += 1;
            j += 1;
        }
        ...
        // SG patterns
        j = 0;
        while j < SG_PATTERNS.len() {
            result[i] = PiiPattern {
                name: SG_PATTERNS[j].name,
                entity_type: SG_PATTERNS[j].entity_type,
                pattern: SG_PATTERNS[j].pattern,
                score: SG_PATTERNS[j].score,
                context_keywords: SG_PATTERNS[j].context_keywords,
                context_required: SG_PATTERNS[j].context_required,
            };
            i += 1;
            j += 1;
        }
        ...
        // SI patterns
        j = 0;
        while j < SI_PATTERNS.len() {
            result[i] = PiiPattern {
                name: SI_PATTERNS[j].name,
                entity_type: SI_PATTERNS[j].entity_type,
                pattern: SI_PATTERNS[j].pattern,
                score: SI_PATTERNS[j].score,
                context_keywords: SI_PATTERNS[j].context_keywords,
                context_required: SI_PATTERNS[j].context_required,
            };
            i += 1;
            j += 1;
        }
        ...
        // TH patterns
        j = 0;
        while j < TH_PATTERNS.len() {
            result[i] = PiiPattern {
                name: TH_PATTERNS[j].name,
                entity_type: TH_PATTERNS[j].entity_type,
                pattern: TH_PATTERNS[j].pattern,
                score: TH_PATTERNS[j].score,
                context_keywords: TH_PATTERNS[j].context_keywords,
                context_required: TH_PATTERNS[j].context_required,
            };
            i += 1;
            j += 1;
        }
...
mod tests {
    ...
    fn test_entity_types_count() {
        ...
        // Current count: 63 unique entity types
        assert_eq!(
            entity_types.len(),
            63,
    ...
    fn test_validators_accessible() {
        ...
        let _ = valid_kr_rrn("850101-1234566");
        ...
        let _ = valid_th_tnin("1123456789014");
    }
