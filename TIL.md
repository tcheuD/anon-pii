...
## Agent Architecture: Split on Conflict of Interest, Not Task Count

When designing multi-agent workflows, the split boundary should be **conflicting responsibilities**, not arbitrary task decomposition. An agent that implements code AND reviews its own code has a natural bias toward "I'm done, commit" — it wrote the code, it thinks it's fine. Caps-lock instructions (`CRITICAL: DO NOT SKIP THIS STEP`) are duck tape on an architectural problem; the real fix is tool-level enforcement. A read-only reviewer agent with no write tools *literally cannot* skip the review or silently fix things. Same-responsibility tasks (implement + test + stage + commit) belong in one agent — no conflict. Conflicting responsibilities (produce code vs. critique code) need separate agents with different permission sets. The litmus test: if your prompt needs `MUST` or `IMPORTANT` to force a step, that step probably belongs to a different agent.
